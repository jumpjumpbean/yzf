	...
	var imageDialog = function( editor, dialogType ) {
			...
			var resetSize = function( dialog ) {
					...
					if ( oImageOriginal.getCustomData( 'isReady' ) == 'true' ) {
                        ...
                        var w = oImageOriginal.$.width,h = oImageOriginal.$.height;
                        if(w > 600){
                            h = Math.floor(600 * h / w);
                            w = 600;
                        }
						widthField && widthField.setValue( w);
						heightField && heightField.setValue( h );
					}
					updatePreview( dialog );
				};
					...
					elements: [
						...
						children: [
							...
							children: [
								...
								align: 'center',
								action: contextPath+"user/upload?isTemp=99",
								hidden: false,
								onChange:function(e){
									var dialog = this.getDialog();
									if(uplock == 0){
										...
										var formObj = new CKEDITOR.dom.element( frameDocument.$.forms[ 0 ]);
//										formObj.$.submit();
										$(formObj.$).ajaxSubmit({
										    ...
										    success: function(json) {
										    	if(json.url){
										    		dialog.setValueOf('info','txtUrl',contextPath +"upld/"+ json.url);
										    	}else if(json.msg){
                                                    showMessage(json.msg,"error");
                                                }
											},
										    error:function(XMLHttpRequest, textStatus, errorThrown){
										    	if((errorThrown && errorThrown.toString().indexOf("Too Large") > 0) || (XMLHttpRequest && XMLHttpRequest.responseText && XMLHttpRequest.statusText.toString().indexOf("Too Large") > 0)){
                                                    showMessage("图片太大导致上传失败","error");
										    	}else{
                                                    showMessage("图片上传过程中发生错误，请稍后再试!","error");
										    	}
											},
											complete:function(){
												uplock = 0;
